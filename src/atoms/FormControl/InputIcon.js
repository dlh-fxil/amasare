...
export default function InputIcon({ placeholder, color, size, outline, error, success, iconFamily, iconName, ...rest }) {
	let iconUI,
		iconSize,
		labelBorderColor,
		mtInputBorderColor,
	...
	const sharedClasses = ["w-full", "h-full", "text-gray-800", "leading-normal", "shadow-none", "outline-none", "focus:outline-none", "focus:ring-0", "focus:text-gray-800"];

	const inputSM = [...sharedClasses, `${outline ? "pl-2 pr-7" : "pl-0 pr-6"}`, `${outline && "pt-1.5 pb-0.5"}`, "text-sm"];
	const inputRegular = [...sharedClasses, `${outline ? "pl-3 pr-9" : "pl-0 pr-7"}`, `${outline && "pt-2.5 pb-1.5"}`];
	const inputLG = [...sharedClasses, `${outline ? "pl-3 pr-9" : "pl-0 pr-7"}`, `${outline && "pt-3.5 pb-2.5"}`];

	const inputFilled = [mtInputBorderColor, "mt-input", "bg-transparent", "border", "border-none"];
	...
	return (
		<div className={container}>
			...
			<input {...rest} placeholder=" " className={`${inputClasses} ${error && "mt-input-outline-error"} ${success && "mt-input-outline-success"}`} />
			<label className={label}>{outline ? placeholder : <span className={`${size === "sm" && "text-sm"} absolute top-1/4 transition-all duration-300`}>{placeholder}</span>}</label>
