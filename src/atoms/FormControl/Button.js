import React, { forwardRef } from "react";
import Ripple from "material-ripple-effects";

const filledBgColors = {
	slate: "bg-slate-800",
	gray: "bg-gray-500",
	rose: "bg-rose-500",
	violet: "bg-deep-orange-500",
	orange: "bg-orange-500",
	amber: "bg-amber-600",
	yellow: "bg-yellow-600",
	lime: "bg-lime-700",
	emerald: "bg-emerald-500",
	green: "bg-green-500",
	teal: "bg-teal-500",
	cyan: "bg-cyan-500",
	sky: "bg-sky-900",
	blue: "bg-blue-900",
	indigo: "bg-indigo-800",
	stone: "bg-stone-500",
	purple: "bg-purple-700",
	pink: "bg-pink-500",
	red: "bg-red-700",
};

const filledBgHoverColors = {
	slate: "hover:bg-slate-700",
	gray: "hover:bg-gray-700",
	rose: "hover:bg-rose-700",
	violet: "hover:bg-deep-orange-700",
	orange: "hover:bg-orange-700",
	amber: "hover:bg-amber-700",
	yellow: "hover:bg-yellow-700",
	lime: "hover:bg-lime-700",
	emerald: "hover:bg-emerald-700",
	green: "hover:bg-green-700",
	teal: "hover:bg-teal-700",
	cyan: "hover:bg-cyan-700",
	sky: "hover:bg-sky-700",
	blue: "hover:bg-blue-700",
	indigo: "hover:bg-indigo-700",
	stone: "hover:bg-stone-700",
	purple: "hover:bg-purple-700",
	pink: "hover:bg-pink-700",
	red: "hover:bg-red-700",
};

const filledBgFocusColors = {
	slate: "focus:bg-slate-400",
	gray: "focus:bg-gray-400",
	rose: "focus:bg-rose-400",
	violet: "focus:bg-deep-orange-400",
	orange: "focus:bg-orange-400",
	amber: "focus:bg-amber-400",
	yellow: "focus:bg-yellow-500",
	lime: "focus:bg-lime-400",
	emerald: "focus:bg-emerald-400",
	green: "focus:bg-green-400",
	teal: "focus:bg-teal-400",
	cyan: "focus:bg-cyan-400",
	sky: "focus:bg-sky-400",
	blue: "focus:bg-blue-400",
	indigo: "focus:bg-indigo-400",
	stone: "focus:bg-stone-400",
	purple: "focus:bg-purple-400",
	pink: "focus:bg-pink-400",
	red: "focus:bg-red-400",
};

const filledBgActiveColors = {
	slate: "active:bg-slate-800",
	gray: "active:bg-gray-800",
	rose: "active:bg-rose-800",
	violet: "active:bg-deep-orange-800",
	orange: "active:bg-orange-800",
	amber: "active:bg-amber-800",
	yellow: "active:bg-yellow-800",
	lime: "active:bg-lime-800",
	emerald: "active:bg-emerald-800",
	green: "active:bg-green-800",
	teal: "active:bg-teal-800",
	cyan: "active:bg-cyan-800",
	sky: "active:bg-sky-800",
	blue: "active:bg-blue-800",
	indigo: "active:bg-indigo-800",
	stone: "active:bg-stone-800",
	purple: "active:bg-purple-800",
	pink: "active:bg-pink-800",
	red: "active:bg-red-800",
};

const filledShadowColors = {
	slate: "shadow-md-slate",
	gray: "shadow-md-gray",
	rose: "shadow-md-rose",
	violet: "shadow-md-deep-orange",
	orange: "shadow-md-orange",
	amber: "shadow-md-amber",
	yellow: "shadow-md-yellow",
	lime: "shadow-md-lime",
	emerald: "shadow-md-emerald",
	green: "shadow-md-green",
	teal: "shadow-md-teal",
	cyan: "shadow-md-cyan",
	sky: "shadow-md-sky",
	blue: "shadow-md-blue",
	indigo: "shadow-md-indigo",
	stone: "shadow-md-stone",
	purple: "shadow-md-purple",
	pink: "shadow-md-pink",
	red: "shadow-md-red",
};

const filledShadowHoverColors = {
	slate: "hover:shadow-lg-slate",
	gray: "hover:shadow-lg-gray",
	rose: "hover:shadow-lg-rose",
	violet: "hover:shadow-lg-deep-orange",
	orange: "hover:shadow-lg-orange",
	amber: "hover:shadow-lg-amber",
	yellow: "hover:shadow-lg-yellow",
	lime: "hover:shadow-lg-lime",
	emerald: "hover:shadow-lg-emerald",
	green: "hover:shadow-lg-green",
	teal: "hover:shadow-lg-teal",
	cyan: "hover:shadow-lg-cyan",
	sky: "hover:shadow-lg-sky",
	blue: "hover:shadow-lg-blue",
	indigo: "hover:shadow-lg-indigo",
	stone: "hover:shadow-lg-stone",
	purple: "hover:shadow-lg-purple",
	pink: "hover:shadow-lg-pink",
	red: "hover:shadow-lg-red",
};

const outlineTextColors = {
	slate: "text-slate-800",
	gray: "text-gray-500",
	rose: "text-rose-500",
	violet: "text-deep-orange-500",
	orange: "text-orange-500",
	amber: "text-amber-600",
	yellow: "text-yellow-600",
	lime: "text-lime-700",
	emerald: "text-emerald-500",
	green: "text-green-500",
	teal: "text-teal-500",
	cyan: "text-cyan-500",
	sky: "text-sky-900",
	blue: "text-blue-600",
	indigo: "text-indigo-800",
	stone: "text-stone-500",
	purple: "text-purple-700",
	pink: "text-pink-500",
	red: "text-red-500",
};

const outlineBorderColors = {
	slate: "border-slate-800",
	gray: "border-gray-500",
	rose: "border-rose-500",
	violet: "border-deep-orange-500",
	orange: "border-orange-500",
	amber: "border-amber-600",
	yellow: "border-yellow-600",
	lime: "border-lime-700",
	emerald: "border-emerald-500",
	green: "border-green-500",
	teal: "border-teal-500",
	cyan: "border-cyan-500",
	sky: "border-sky-900",
	blue: "border-blue-600",
	indigo: "border-indigo-800",
	stone: "border-stone-500",
	purple: "border-purple-700",
	pink: "border-pink-500",
	red: "border-red-500",
};

const outlineBgHoverColors = {
	slate: "hover:bg-slate-50",
	gray: "hover:bg-gray-50",
	rose: "hover:bg-rose-50",
	violet: "hover:bg-deep-orange-50",
	orange: "hover:bg-orange-50",
	amber: "hover:bg-amber-50",
	yellow: "hover:bg-yellow-50",
	lime: "hover:bg-lime-50",
	emerald: "hover:bg-emerald-50",
	green: "hover:bg-green-50",
	teal: "hover:bg-teal-50",
	cyan: "hover:bg-cyan-50",
	sky: "hover:bg-sky-50",
	blue: "hover:bg-blue-50",
	indigo: "hover:bg-indigo-50",
	stone: "hover:bg-stone-50",
	purple: "hover:bg-purple-50",
	pink: "hover:bg-pink-50",
	red: "hover:bg-red-50",
};

const outlineBorderHoverColors = {
	slate: "hover:border-slate-700",
	gray: "hover:border-gray-700",
	rose: "hover:border-rose-700",
	violet: "hover:border-deep-orange-700",
	orange: "hover:border-orange-700",
	amber: "hover:border-amber-700",
	yellow: "hover:border-yellow-700",
	lime: "hover:border-lime-700",
	emerald: "hover:border-emerald-700",
	green: "hover:border-green-700",
	teal: "hover:border-teal-700",
	cyan: "hover:border-cyan-700",
	sky: "hover:border-sky-700",
	blue: "hover:border-blue-700",
	indigo: "hover:border-indigo-700",
	stone: "hover:border-stone-700",
	purple: "hover:border-purple-700",
	pink: "hover:border-pink-700",
	red: "hover:border-red-700",
};

const outlineTextHoverColors = {
	slate: "hover:text-slate-700",
	gray: "hover:text-gray-700",
	rose: "hover:text-rose-700",
	violet: "hover:text-deep-orange-700",
	orange: "hover:text-orange-700",
	amber: "hover:text-amber-700",
	yellow: "hover:text-yellow-700",
	lime: "hover:text-lime-700",
	emerald: "hover:text-emerald-700",
	green: "hover:text-green-700",
	teal: "hover:text-teal-700",
	cyan: "hover:text-cyan-700",
	sky: "hover:text-sky-700",
	blue: "hover:text-blue-700",
	indigo: "hover:text-indigo-700",
	stone: "hover:text-stone-700",
	purple: "hover:text-purple-700",
	pink: "hover:text-pink-700",
	red: "hover:text-red-700",
};

const outlineBgActiveColors = {
	slate: "active:bg-slate-100",
	gray: "active:bg-gray-100",
	rose: "active:bg-rose-100",
	violet: "active:bg-deep-orange-100",
	orange: "active:bg-orange-100",
	amber: "active:bg-amber-100",
	yellow: "active:bg-yellow-100",
	lime: "active:bg-lime-100",
	emerald: "active:bg-emerald-100",
	green: "active:bg-green-100",
	teal: "active:bg-teal-100",
	cyan: "active:bg-cyan-100",
	sky: "active:bg-sky-100",
	blue: "active:bg-blue-100",
	indigo: "active:bg-indigo-100",
	stone: "active:bg-stone-100",
	purple: "active:bg-purple-100",
	pink: "active:bg-pink-100",
	red: "active:bg-red-100",
};

const Button = (
	{
		loading = false,
		disable = false,
		children,
		color = "sky",
		buttonType = "filled",
		size = "regular",
		rounded = false,
		iconOnly = false,
		block = false,
		ripple = "light",
		className = "",
		...rest
	},
	ref,
) => {
	const rippleEffect = new Ripple();
	let classes = [];
	// Button.defaultProps = {
	// 	color: "sky",
	// 	buttonType: "filled",
	// 	size: "lg",
	// 	rounded: false,
	// 	block: false,
	// };

	// Button.propTypes = {
	// 	children: PropTypes.node.isRequired,
	// 	color: PropTypes.string.isRequired,
	// 	buttonType: PropTypes.string.isRequired,
	// 	size: PropTypes.string.isRequired,
	// 	rounded: PropTypes.bool.isRequired,
	// 	block: PropTypes.bool.isRequired,
	// 	ripple: PropTypes.string,
	// };

	rounded = rounded ? "rounded-full" : "rounded-md";

	const sharedClasses = [
		block && "w-full",
		loading && "cursor-wait",
		disable && "cursor-not-allowed opacity-30",
		"flex",
		"group",
		"items-center",
		"justify-center",
		"gap-1",
		"font-bold",
		"outline-none",
		"uppercase",
		"tracking-wider",
		"focus:outline-none",
		"focus:shadow-none",
		"transition-all",
		"duration-300",
		rounded,
	];

	const buttonFilled = [
		"text-white",
		filledBgColors[color],
		filledBgHoverColors[color],
		filledBgFocusColors[color],
		filledBgActiveColors[color],
		filledShadowColors[color],
		filledShadowHoverColors[color],
	];

	const buttonOutline = [
		"bg-transparent",
		"border",
		"border-solid",
		"shadow-none",
		outlineTextColors[color],
		outlineBorderColors[color],
		outlineBgHoverColors[color],
		outlineBorderHoverColors[color],
		outlineTextHoverColors[color],
		outlineBgHoverColors[color],
		outlineBgActiveColors[color],
	];

	const buttonLink = [
		`bg-transparent`,
		outlineTextColors[color],
		outlineBgHoverColors[color],
		outlineTextHoverColors[color],
		outlineBgHoverColors[color],
		outlineBgActiveColors[color],
	];

	const buttonXS = [
		...sharedClasses,
		iconOnly ? "w-fit h-fit p-0 grid place-items-center" : "py-0.5 px-1",
		"tracking-tightest",
		"font-normal",
		"text-xs",
		"leading-normal",
	];
	const buttonResponsive = [
		...sharedClasses,
		"p-0  items-center rounded-full sm:rounded-md justify-center w-fit sm:py-2 sm:px-4",
		"text-xs",
		"leading-normal",
	];
	const buttonSM = [
		...sharedClasses,
		iconOnly ? "w-fit h-fit p-0 grid place-items-center" : "py-1 px-4",
		"text-xs",
		"leading-normal",
	];
	const buttonMedium = [
		...sharedClasses,
		iconOnly ? "w-fit h-fit p-0 grid place-items-center" : "py-1.5 px-4",
		"text-xs",
		"leading-normal",
	];
	const buttonRegular = [
		...sharedClasses,
		iconOnly ? "w-fit h-fit p-0 grid place-items-center" : "py-2 px-6",
		"text-xs",
		"leading-normal",
	];
	const buttonLG = [
		...sharedClasses,
		iconOnly ? "w-12 h-12 p-0 grid place-items-center" : "py-3 px-7",
		"text-sm",
		"leading-relaxed",
	];

	if (size === "xs") {
		classes.push(...buttonXS);
	} else if (size === "sm") {
		classes.push(...buttonSM);
	} else if (size === "md") {
		classes.push(...buttonMedium);
	} else if (size === "lg") {
		classes.push(...buttonLG);
	} else if (size === "responsive") {
		classes.push(...buttonResponsive);
	} else {
		classes.push(...buttonRegular);
	}

	if (buttonType === "outline") {
		classes.push(...buttonOutline);
	} else if (buttonType === "link") {
		classes.push(...buttonLink);
	} else {
		classes.push(...buttonFilled);
	}

	classes = classes.join(" ");

	return (
		<button
			{...rest}
			className={`${className} ${classes}`}
			ref={ref}
			onMouseUp={e => {
				ripple === "dark" && rippleEffect.create(e, "dark");
				ripple === "light" && rippleEffect.create(e, "light");
			}}>
			{children}
		</button>
	);
};

export default forwardRef(Button);
